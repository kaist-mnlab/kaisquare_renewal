...
var io;
var db = require('mongojs').connect('lectures', ['videos']);
		...
		io.sockets.on('connection', function (socket) {
		    ...
		    socket.on('record', function (data) {
		        ...
		        if (data.video) {
		            ...
		            db.videos.save({ uid: data.info.uid, gid: data.info.gid, address: data.audioName.split('.')[0] + '.webm' });
		        }
		    });
		
		    socket.on('getlecture', function () {
		        db.videos.find(function (error, data) {
		            socket.emit('lecture_list', data);
		        })
		    });
		});
	}
...
function ifWin(socket, data) {
    ...
    var cmd = exec(command, function (error) {
        if (error) {
            ...
            db.videos.save({ uid: data.info.uid, gid: data.info.gid, address: data.audioName.split('.')[0] + '-merged.webm' });
